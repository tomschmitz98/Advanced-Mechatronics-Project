...
#define VALID_CLK_INTERRUPTS (INTERNAL_LOW_SPEED_CLK | EXTERNAL_LOW_SPEED_CLK | INTERNAL_HIGH_SPEED_CLK | \
		EXTERNAL_HIGH_SPEED_CLK | MAIN_PLL_CLK | I2S_PLL_CLK | SAI_PLL_CLK | CLOCK_SECURITY_SYSTEM)

#define VALID_RCC_AHB1ENR_MASK (UPPER16BITS(BITE | BITD | BIT6 | BIT5 | BIT2) | BITC)
#define VALID_RCC_AHB2ENR_MASK BIT7 | BIT0
#define VALID_RCC_AHB3ENR_MASK BIT1 | BIT0
#define VALID_RCC_APB1ENR_MASK (UPPER16BITS(BITD | BITC | BITB | BITA | BIT9 | BIT8 | BIT7 | BIT6 | BIT5 | \
		BIT4 | BIT3 | BIT2 | BIT1 |BIT0) | BITF | BITE | BITB | BIT8 | BIT7 | BIT6 | BIT5 | BIT4 | BIT3 | \
		BIT2 | BIT1 | BIT0)
#define VALID_RCC_APB2ENR_MASK (UPPER16BITS(BIT7 | BIT6 | BIT2 | BIT1 | BIT0) | BITE | BITD | BITC | \
		BITB | BITA | BIT9 | BIT8 | BIT5 | BIT4 | BIT1 | BIT0)

#define INCREMENT_MASK (UPPER16BITS(BITB | BITA | BIT9 | BIT8 | BIT7 | BIT6 | BIT5 | BIT4 | BIT3 | \
		BIT2 | BIT1 | BIT0) | BITF | BITE | BITD)
#define MODULATION_MASK (BITC | BITB | BITA | BIT9 | BIT8 | BIT7 | BIT6 | BIT5 | BIT4 | BIT3 | BIT2 | BIT1 | BIT0)
...
{
	assert(pll == SERIAL_AUDIO_INTERFACE_PLL || pll == I2S_PLL || pll == MAIN_PLL);
...
void enable_peripheral_low_power_clock(uint32_t reg, uint32_t peripheral)
{
	...
	switch(reg)
	{
	case 0: offset = RCC_AHB1LPENR;
		valid_mask = VALID_RCC_AHB1ENR_MASK;
		break;
	case 1: offset = RCC_AHB2LPENR;
		valid_mask = VALID_RCC_AHB2ENR_MASK;
		break;
	case 2: offset = RCC_AHB3LPENR;
		valid_mask = VALID_RCC_AHB3ENR_MASK;
		break;
	case 3: offset = RCC_APB1LPENR;
		valid_mask = VALID_RCC_APB1ENR_MASK;
		break;
	case 4: offset = RCC_APB2LPENR;
		valid_mask = VALID_RCC_APB2ENR_MASK;
		break;
	default: assert(0);
		break;
...
{
	if (CHECK_BIT(RCC_BASE(RCC_CR), 28) || CHECK_BIT(RCC_BASE(RCC_CR), 26) || CHECK_BIT(RCC_BASE(RCC_CR), 24))
		return;
